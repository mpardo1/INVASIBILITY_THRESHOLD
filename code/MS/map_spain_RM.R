...
vec <- seq(0,20,0.1)
out <- sapply(vec, h_f_jap, hum = 0.4)
...
# Join landcover ---------------------------------------------------
path <- "~/INVASIBILITY_THRESHOLD/data/landcover_perc_broaded-leaved.Rds"
landcover <- readRDS(path)[,c("NATCODE", "freq")]
ggplot(landcover) + geom_sf(aes(fill = freq), color = NA)
landcover$geometry <- NULL
