...
# test 
ggplot(clim_pop[clim_pop$month == 6,]) +
  geom_raster(aes(x = lon, y = lat, 
                  fill = R0_cul),alpha = 1) +
  scale_fill_viridis_c() + theme_minimal()

# Save monthly estimates
saveRDS(clim_pop[,c("id" ,"month","lon","lat","R0_cul")],"~/INVASIBILITY_THRESHOLD/data/ERA5/Europe/eu_monthly_culex_2020.Rds")
write.csv(clim_pop[,c("id" ,"month","lon","lat","R0_cul")],
          "~/Documentos/PHD/2024/Culex_RM/eu_monthly_culex_2020.csv")

# Read past computed culex R0 monthly
clim_pop <- readRDS("~/Documentos/PHD/2024/Culex_RM/RM_eu_monthly_culex_2020.Rds")
head(clim_pop)
max(clim_pop$R0_cul, na.rm=TRUE)
ggplot(clim_pop) +
  geom_point(aes(tmean,R0_cul))
...
# Annual estimates
saveRDS(clim_pop,"~/INVASIBILITY_THRESHOLD/data/ERA5/Europe/eu_NumSuitableMonths_culex_2020.Rds")
write.csv(clim_pop,"~/INVASIBILITY_THRESHOLD/data/ERA5/Europe/eu_NumSuitableMonths_culex_2020.csv")
