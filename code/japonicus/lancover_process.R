...
filt_land <- landcov_fracs[landcov_fracs$value == categories_of_interest,]
write.csv(filt_land, "~/INVASIBILITY_THRESHOLD/data/landcover_perc.csv")
saveRDS(filt_land,"~/INVASIBILITY_THRESHOLD/data/landcover_perc.Rds")
...
ggplot(filt_land) + 
  ...
  scale_fill_viridis_c(name = paste0("Percentage \n",
                                     df_cat[df_cat$Value == categories_of_interest, "LABEL3"])) +
  theme_bw()
